...
namespace Samples
{
    public class EyewearDeviceParameters : MonoBehaviour
    {
        ...
        private Quaternion displayCompensation;
        ...
        {
            if (!Session) { return; }
            var cameraDevice = Session.GetComponentInChildren<CameraDeviceFrameSource>();
            ...
            if (deviceModel == parameterList[eyewearActionOne][0].DeviceModel)
            {
                ...
                {
                    LeftEye.projectionMatrix = parameterList[eyewearActionOne][0].Projection(Session, LeftEye) * Matrix4x4.Rotate(Quaternion.Inverse(displayCompensation));
                    RightEye.projectionMatrix = parameterList[eyewearActionOne][1].Projection(Session, RightEye) * Matrix4x4.Rotate(Quaternion.Inverse(displayCompensation));
                };
            }
